...
//imports the json question data to the server file, allowing the server to just sent that data back to the user with the required params.
const banki = require(`${__dirname}/dev-data/banki.js`);
...
const server = http.createServer((req, res) => {
  ...
  //this is the test api for banki questions. This is just to look at all the api data we use lines 34-37 dont really matter
    ...
    if ("question" in params) {
      ...
      res.end(JSON.stringify(banki[`${params["question"]}`]));
      ...
      res.end(
        JSON.stringify(banki.filter((x) => x.category == params["category"]))
      );
    }
  }
